...
- **`lv_demo_widgets.c`**: Orchestrates the root `lv_tabview`, determines display scaling (`disp_size == DISP_LARGE`), and manages header logo/title widgets.
- **`lv_demo_widgets_profile.c`**: User metadata forms, status switches, sliders, and avatar image handling.
- **`lv_demo_widgets_analytics.c`**: Complex gauges (`lv_scale`), line charts (`lv_chart`), and real-time metric updates.
- **`lv_demo_widgets_shop.c`**: E-commerce style item grid cards and action buttons.
- **`lv_demo_widgets_components.c`**: Shared design tokens, styles, and reusable custom card containers.
...
In `lv_demo_music_main.c`, the spectrum analyzer does not use static widgets. Instead, it registers a custom draw event callback (`spectrum_draw_event_cb` on `LV_EVENT_DRAW_MAIN_BEGIN`):
...
`lv_demo_benchmark.c` measures rendering throughput across 20+ distinct visual scenes (solid fills, opacity layers, rotated text, blitted images, arc gauges).
...
`lv_demo_stress.c` runs an automated state machine (`g_state`) inside `obj_test_task_cb` every `LV_DEMO_STRESS_TIME_STEP` (default 50ms):
